...
from openprocurement.auctions.sspinsider.models import DGFInsider
...
from openprocurement.auctions.sspinsider.tests.blanks.tender_blanks import (
    # InsiderAuctionTest
    create_role,
    edit_role,
...
class InsiderAuctionProcessTest(BaseInsiderAuctionWebTest):
    ...
    def test_auctionParameters(self):
        ...
        self.assertEqual(response.json['errors'], [
            {
                "location": "body", "name": "auctionParameters", "description": {
                    "type": ["Value must be one of ['sspinsider']."],
                    "dutchSteps": ["Value must be one of [10, 20, 30, 40, 50, 60, 70, 80, 90, 99, 100]."]
                }
            }
        ])

        data['auctionParameters'] = {'dutchSteps': 112, 'type': 'sspinsider'}
        response = self.app.post_json('/auctions', {'data': data}, status=422)
        self.assertEqual(response.status, '422 Unprocessable Entity')
        self.assertEqual(response.content_type, 'application/json')
        self.assertEqual(response.json['errors'], [
        ...
        self.assertEqual(response.json['data']['auctionParameters']['type'], 'sspinsider')

        data['auctionParameters'] = {'dutchSteps': 70, 'type': 'sspinsider'}
        response = self.app.post_json('/auctions', {'data': data})
        self.assertEqual(response.status, '201 Created')
        self.assertEqual(response.content_type, 'application/json')
        ...
        response = self.app.patch_json('/auctions/{}?acc_token={}'.format(auction_id, owner_token), {
            'data': {'auctionParameters': {'dutchSteps': 50, 'type': 'sspinsider'}}
        })
        ...
        response = self.app.patch_json('/auctions/{}'.format(auction_id), {
            'data': {'auctionParameters': {'dutchSteps': 99, 'type': 'sspinsider'}}
        })
        self.assertEqual(response.status, '200 OK')
        self.assertEqual(response.content_type, 'application/json')
